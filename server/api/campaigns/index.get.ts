...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (query.userId != undefined) {
      const response: CampaignDto[] =
        await campaignService.getCampaignsByUserId(parseInt(query.userId));
      return {
        data: response,
        count: response.length,
      };
    } else if (query.campaignId != undefined) {
      const response: CampaignDto = await campaignService.getCampaignById(
        parseInt(query.campaignId)
      );
      return {
        data: response,
      };
